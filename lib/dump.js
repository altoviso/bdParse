...
symbols["asntFor"].dump= function(children) {
  return {for: 1, init:dump(children[0][0]), test:dump(children[1][0]), step:dump(children[2]), statement:dump(children[3])};
};

symbols["asntFunctionDef"].dump= function(children) {
  return {function:children[0].value, params:children[1].map(function(item){return item[0].value;}), body:dump(children[2])};
};
...
symbols["asntBinaryOp"].dump= function(children) {
  return ({}[children[0].value]= [dump(children[1]), (children[0]=="(" ? children[2].map(dump) : dump(children[2]))]);
};
