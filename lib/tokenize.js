...
var
  ...
  tokenize= function(source) {
    var
      ...
      operatorRe= /^((((in(stanceof)?)|(typeof)|(new)|(void)|(delete))([^a-zA-Z]|$))|[^a-zA-Z])/,
      ...
      readWord= function() {
        ...
        if (match) {
          match= match[0];
          col+= match.length;
          if (!keywords.hasOwnProperty(match)) {
            return makeToken(tName, match);
          } else if (operators.hasOwnProperty(match)) {
            return makeToken(tOperator, match);
          } else {
      ...
      nextToken= function() {
        ...
        if (!nextChar) {
          ...
          throw new error(line, col, "unexpected token");
        }
      };
    ...
    try {
      ...
      console.log("tokenizer found an error");
      console.log(e);
      return e;
    }
  };
